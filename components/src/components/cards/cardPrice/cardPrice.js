...
class CardPrice extends React.Component {
    ...
    makePrice(isSale) {
        if (this.isPriceInvalid())
            return React.createElement("p", { className: "card__price" }, " -");
        if (isSale) {
            return (React.createElement(React.Fragment, null,
                React.createElement("p", { className: "card__price card__price_sale" }, this.makePriceWithCents(this.makeNewPrice())),
                React.createElement("span", { className: "card__price card__price_old" }, this.makePriceWithCents(this.price))));
        }
        return React.createElement("p", { className: "card__price" }, this.makePriceWithCents(this.price));
    }
    isSale() {
        return this.sale > 0 && this.sale <= 99;
    }
    render() {
        return (React.createElement("div", { className: "card__prices" },
